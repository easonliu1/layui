...
layui.define(['layer', 'laypage','form'], function(exports) {
	...
	begTable.prototype.init = function() {
		...
		//添加全选按钮
		if(_config.checked) {
			theadTemp += '<th style="width:25px;text-align:center;"><input type="checkbox"  lay-skin="primary" lay-filter="allChoose"></th>'
		}
		...
		if(data !== undefined && typeof(data) === 'object') {
			for(var i = 0; i < data.length; i++) {
				var tr = '<tr>';
				if(_config.checked) {
					tr += '<td style="text-align: center;"><input type="checkbox" lay-skin="primary" ids="'+ data[i].id +'" /></td>';
				}
				for(var j = 0; j < columns.length; j++) {
					tr += '<td style="text-align: center;">' + data[i][columns[j].field] + '</td>';
				}
				if(_config.operation){
					tr += '<td style="text-align:center;">'+
                            '<a href="javascript:;" target="_blank" class="layui-btn layui-btn-normal layui-btn-mini" onclick="layuiLook(this)">预览</a>'+
                            '<a href="javascript:;" class="layui-btn layui-btn-mini" onclick="layuiEdit(this)">编辑</a>'+
                            '<a href="javascript:;" data-id="1" data-opt="del" class="layui-btn layui-btn-danger layui-btn-mini" onclick="layuiDel(this)">删除</a>'+
                        '</td>'
				}
				tr += '</tr>';
				tbodyTemp += tr;
